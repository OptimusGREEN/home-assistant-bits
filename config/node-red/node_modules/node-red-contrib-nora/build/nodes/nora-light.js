...
module.exports = function (RED) {
    RED.nodes.registerType('nora-light', function (config) {
        ...
        rxjs_1.combineLatest(device$, state$)
            ...
            return device.updateState(tslib_1.__assign({}, state));
        });
        ...
        this.on('input', function (msg) {
            ...
            if (!brightnessControl) {
                ...
                if (RED.util.compareObjects(myOnValue, msg.payload)) {
                    state$.next(tslib_1.__assign({}, state$.value, { on: true }));
                }
                else if (RED.util.compareObjects(myOffValue, msg.payload)) {
                    state$.next(tslib_1.__assign({}, state$.value, { on: false }));
                }
            }
            else {
                if (statepayload) {
                    if (typeof msg.payload !== 'object' || !msg.payload) {
                        _this.error('Payload must be an object like { [on]: true/false, [brightness]: 0-100 }');
                    }
                    else {
                        var state = tslib_1.__assign({}, state$.value);
                        var update = false;
                    ...
                    if (isFinite(brightness)) {
                        if (brightness === 0) {
                            if (brightnessOverride !== 0) {
                                state$.next(tslib_1.__assign({}, state$.value, { on: false, brightness: brightnessOverride }));
                            }
                            else {
                                state$.next(tslib_1.__assign({}, state$.value, { on: false }));
                            }
                            ...
                            state$.next(tslib_1.__assign({}, state$.value, { on: true, brightness: brightness }));
                        }
                    }
                    else {
